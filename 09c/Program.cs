...
namespace _09c
{
  ...
  class VM
  {
    ...
    private long ReadValue(List<long> inputData, int curPos, int parameterPosition)
    {
      var opCode = ToArray(inputData[curPos]);
      long val = 0;
      var address = inputData[curPos + parameterPosition];
      int shifted = parameterPosition + 1;

      if (opCode.Length >= shifted && opCode[shifted] == 1)
        val = inputData[curPos + parameterPosition];
      else if (opCode.Length >= shifted && opCode[shifted] == 2)
        val = inputData[(int)referenceBase];
      else
        val = (inputData.Count < address) ? 0 : inputData[(int)address];

      return val;
    }

    private static int[] ToArray(long number)
    {
